// ============================================================
// RemindMe Pro — Settings Page
// UPDATED: Added "Disable Notifications" toggle
// ============================================================

import { getTheme, setTheme } from '../services/theme.js';
import { getAllTasks, deleteTask } from '../services/db.js';
import { requestPermission, cancelReminders } from '../services/notifications.js';
import { showToast } from '../utils/helpers.js';
import {
    exportAsJSON, exportAsCSV, importFromJSON, restoreFromBackup,
    autoBackup, getAutoBackups, restoreAutoBackup,
    isAutoBackupEnabled, setAutoBackupEnabled, startAutoBackup, stopAutoBackup
} from '../services/backup.js';

// NEW: Notification disable helpers
const isNotifDisabled = () => localStorage.getItem('remindme-notifications-disabled') === 'true';
const setNotifDisabled = (v) => localStorage.setItem('remindme-notifications-disabled', v ? 'true' : 'false');
...
export const renderSettings = async () => {
    ...
    let tasks = [];
    try { tasks = await getAllTasks(); } catch (e) {}
    const autoOn = isAutoBackupEnabled();
    const autoBackups = getAutoBackups();
    const notifDisabled = isNotifDisabled(); // NEW
    ...
    content.innerHTML = `<div class="fade-in">
        ...
        <div class="settings-section"><h3>🎨 Appearance</h3>
            <div class="setting-item">
                ...
                <label class="toggle"><input type="checkbox" id="theme-toggle" ${isDark ? 'checked' : ''}><span class="toggle-slider"></span></label>
            </div>
        </div>
        ...
        <div class="settings-section"><h3>🔔 Notifications</h3>
            ...
            <div class="setting-item">
                <div>
                    <div class="setting-label">Disable All Notifications</div>
                    <div class="setting-desc">Turn off all reminder notifications and sounds</div>
                </div>
                <label class="toggle">
                    <input type="checkbox" id="notif-disable-toggle" ${notifDisabled ? 'checked' : ''}>
                    <span class="toggle-slider ${notifDisabled ? 'toggle-danger' : ''}"></span>
                </label>
            </div>
            <div class="setting-item" ${notifDisabled ? 'style="opacity:0.4;pointer-events:none"' : ''}>
                <div><div class="setting-label">Browser Permission</div><div class="setting-desc">Allow browser push notifications</div></div>
                <button class="btn btn-sm btn-primary" id="enable-notif-btn">Enable</button>
            </div>
            <div class="setting-item">
                <div class="setting-label">Permission Status</div>
                <div id="notif-status">${notifDisabled ? '🔇 Disabled by user' : (typeof Notification !== 'undefined' ? Notification.permission : 'Not supported')}</div>
            </div>
        </div>
        ...
        <div class="settings-section"><h3>ℹ️ About</h3>
            <div class="setting-item"><div class="setting-label">RemindMe Pro</div><div>v1.1.0</div></div>
    ...
    document.getElementById('theme-toggle').addEventListener('change', e => {
        ...
        showToast(e.target.checked ? '🌙 Dark mode' : '☀️ Light mode');
    });

    // ============================================================
    // NEW: Disable Notifications Toggle
    // ============================================================
    document.getElementById('notif-disable-toggle').addEventListener('change', async (e) => {
        const disabled = e.target.checked;
        setNotifDisabled(disabled);

        if (disabled) {
            // Cancel ALL active notification timers
            const allTasks = await getAllTasks();
            allTasks.forEach(t => cancelReminders(t.id));
            showToast('🔇 All notifications disabled');
        } else {
            showToast('🔔 Notifications re-enabled — restart app to reschedule');
        }
    ...
    document.getElementById('enable-notif-btn').addEventListener('click', async () => {
        ...
        document.getElementById('notif-status').textContent = g ? 'granted' : 'denied';
        showToast(g ? '🔔 Enabled!' : '❌ Denied');
    });

    // Export JSON
    document.getElementById('export-json-btn').addEventListener('click', async () => {
        try { const c = await exportAsJSON(); showToast(`📤 Exported ${c} reminders`); } catch (e) { showToast('❌ ' + e.message); }
    });

    // Export CSV
    document.getElementById('export-csv-btn').addEventListener('click', async () => {
        try { const c = await exportAsCSV(); showToast(c === 0 ? '⚠️ No reminders' : `📤 Exported as CSV`); } catch (e) { showToast('❌ ' + e.message); }
    });
    ...
    fileInput.addEventListener('change', async (e) => {
        ...
        try {
            ...
            const mode = confirm(`Found ${data.taskCount} reminders.\n\nOK = Merge\nCancel = Replace All`) ? 'merge' : 'replace';
            if (mode === 'replace' && !confirm('Replace ALL current data?')) { fileInput.value = ''; return; }
    ...
    document.getElementById('auto-backup-toggle').addEventListener('change', e => {
        ...
        if (e.target.checked) { startAutoBackup(); showToast('🔄 Auto-backup ON'); }
        else { stopAutoBackup(); showToast('Auto-backup OFF'); }
    });
    ...
    document.querySelectorAll('.restore-auto-btn').forEach(b => b.addEventListener('click', async () => {
        if (confirm('Replace all data with this backup?')) {
            try { const r = await restoreAutoBackup(parseInt(b.dataset.index), 'replace'); showToast(`📥 Restored ${r.imported}`); renderSettings(); }
    ...
    document.getElementById('clear-btn').addEventListener('click', async () => {
        ...
        if (c.length === 0) { showToast('Nothing to clear'); return; }
        for (const t of c) await deleteTask(t.id);
        showToast(`🗑️ Cleared ${c.length}`);
        renderSettings();
    });
    ...
    document.getElementById('delete-all-btn').addEventListener('click', async () => {
        if (confirm('Delete ALL reminders?')) {
            ...
            showToast('🗑️ All deleted (auto-backup saved)');
            renderSettings();
        }
    });
};
...
function fmtDate(d) {
    try { return new Intl.DateTimeFormat('en-IN', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true }).format(new Date(d)); }
    catch (e) { return d; }
}
